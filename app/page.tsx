...
import { Thermometer, Droplet, Sun, Sprout, PhoneCall, PhoneOff, Smile, Meh, Frown, Laugh, Flame, Zap, Cloud, HelpCircle, ThumbsDown, Coffee, Utensils, Heart, CloudRain, Eye, AlertTriangle } from 'lucide-react'

// Mock data for plant metrics
const plantMetricsData = [
  { timestamp: '00:00', soilMoisture: 30, waterLevel: 50, temperature: 22, humidity: 60, lightIntensity: 800, wateringEvents: 0 },
  { timestamp: '04:00', soilMoisture: 28, waterLevel: 48, temperature: 20, humidity: 65, lightIntensity: 0, wateringEvents: 0 },
  { timestamp: '08:00', soilMoisture: 25, waterLevel: 45, temperature: 23, humidity: 55, lightIntensity: 1200, wateringEvents: 1 },
  { timestamp: '12:00', soilMoisture: 35, waterLevel: 60, temperature: 26, humidity: 50, lightIntensity: 1500, wateringEvents: 0 },
  { timestamp: '16:00', soilMoisture: 32, waterLevel: 58, temperature: 25, humidity: 52, lightIntensity: 1000, wateringEvents: 0 },
  { timestamp: '20:00', soilMoisture: 30, waterLevel: 55, temperature: 24, humidity: 58, lightIntensity: 200, wateringEvents: 1 },
]
...
interface Message {
  role: 'user' | 'ai'
  content: string
  sentiment?: 'Adoration/Joy' | 'Amusement' | 'Anger' | 'Awe/Surprise' | 'Calmness' | 'Confusion' | 'Contempt/Pride' | 'Contentment' | 'Craving' | 'Desire/Love' | 'Disappointment/Shame' | 'Distress/Disgust' | 'Fear' | 'Interest' | 'Pain/Sadness' | 'Negative' | 'Neutral'
}
...
export default function Dashboard() {
    ...
    const [messages, setMessages] = useState<Message[]>([
      { role: 'ai', content: "Welcome to Bloom AI. I'm here to assist you with plant care and mindfulness. How can I help you cultivate serenity today?", sentiment: 'positive' }
    ])

  const toggleConnection = () => {
    if (!isConnected) {
      startLiveTranscription()
      setMessages(prev => [...prev, { role: 'ai', content: "Connection established. How can I assist you with your plant care and mindfulness journey today?", sentiment: 'Adoration/Joy' }])
    } else {
      setMessages(prev => [...prev, { role: 'ai', content: "Thank you for using Bloom AI. Remember, like a garden, mindfulness grows with daily care. Until next time!", sentiment: 'Amusement' }])
    }
  ...
  const getSentimentInfo = (sentiment?: string) => {
    switch (sentiment) {
      ...
      case 'contempt':
      case 'pride':
        return { icon: ThumbsDown, color: 'text-orange-500', label: 'Contempt/Pride' }
      case 'contentment':
      ...
      case 'disappointment':
      case 'shame':
        return { icon: CloudRain, color: 'text-blue-600', label: 'Disappointment/Shame' }
      case 'distress':
      case 'disgust':
        return { icon: Frown, color: 'text-green-700', label: 'Distress/Disgust' }
      case 'fear':
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <Card className="rounded-2xl shadow-lg overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-green-500 to-blue-500 text-white">
            <CardTitle className="text-xl sm:text-2xl">Bloom AI Voice Assistant</CardTitle>
          </CardHeader>
          <CardContent className="p-4 sm:p-6">
            <ScrollArea className="h-[300px] sm:h-[400px] w-full mb-4">
              {messages.map((msg, index) => {
                const { icon: SentimentIcon, color, label } = getSentimentInfo(msg.sentiment)
                return (
                  <div key={index} className={`mb-4 ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
                    <div className={`inline-block max-w-[80%] sm:max-w-md p-3 sm:p-4 rounded-2xl ${
            ...
            <div className="flex justify-center">
              ...
              >
                {isConnected ? <PhoneOff className="w-6 h-6 sm:w-8 sm:h-8" /> : <PhoneCall className="w-6 h-6 sm:w-8 sm:h-8" />}
              </Button>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
              ...
              <Card className="bg-yellow-100 rounded-xl">
                <CardContent className="p-4 flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-yellow-900">800 lux</p>
                  </div>
                </CardContent>
              </Card>
            ...
            <Tabs defaultValue="soilMoisture">
              <TabsList className="grid w-full grid-cols-3 rounded-xl bg-green-100">
                <TabsTrigger value="soilMoisture" className="data-[state=active]:bg-green-500 data-[state=active]:text-white">Soil Moisture</TabsTrigger>
                <TabsTrigger value="waterLevel" className="data-[state=active]:bg-green-500 data-[state=active]:text-white">Water Level</TabsTrigger>
                <TabsTrigger value="wateringEvents" className="data-[state=active]:bg-green-500 data-[state=active]:text-white">Watering Events</TabsTrigger>
              </TabsList>
                      ...
                      >
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={plantMetricsData}>
                            ...
                            <Legend />
                            <Line type="monotone" dataKey={metric} stroke="hsl(142, 76%, 36%)" activeDot={{ r: 8 }} />
                          </LineChart>
                        </ResponsiveContainer>
                      </ChartContainer>
                    </CardContent>
